...
class Data(Dataset):
	def __init__(self, root, transform=None):
		...
		for i in file_list:
			...
			width = img.shape[0]
			height = img.shape[1]
			img = img[int(width / 2) - 25: int(width / 2) + 25, int(height / 2) - 25: int(height / 2) + 25]
			for j in range(0, 10):
				if(i[0] == str(j)):
...
# 训练集
train_data = Data('./number_data/training/', transform=transform)

# 测试集
test_data = Data('./number_data/testing/', transform=transform)
